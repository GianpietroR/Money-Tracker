...
function aggiornaTabella() {
    ...
    let saldoTotale = saldoBase;
    let saldoMese = 0;

    const catTotali = {};

    spese.forEach((riga, i) => {
        const entrata = parseFloat(riga.entrate || 0);
        const uscita = parseFloat(riga.uscite || 0);
        const meseMatch = !meseFiltro || riga.mese === meseFiltro;
        ...
        if (meseMatch && catMatch) {
            ...
            btnMod.onclick = () => modificaRiga(i);
            const btnDel = document.createElement("button");
            btnDel.textContent = "🗑️";
            btnDel.onclick = () => eliminaRiga(i);
            ...
            saldoMese += entrata - uscita;

            if (!catTotali[riga.categoria]) catTotali[riga.categoria] = 0;
            catTotali[riga.categoria] += uscita;
        }

        saldoTotale += entrata - uscita;
    });

    document.getElementById("saldo").textContent = "Saldo: €" + saldoTotale.toFixed(2);
    document.getElementById("saldo-mese").textContent = "Saldo del mese: €" + saldoMese.toFixed(2);
    document.getElementById("saldo-totale").textContent = "Saldo totale: €" + (saldoTotale - saldoBase).toFixed(2);

    aggiornaGrafico(catTotali);
}
...
function modificaRiga(index) {
    ...
    spese.splice(index, 1); // rimuoviamo e ricreiamo all'invio
    localStorage.setItem("spese", JSON.stringify(spese));
    aggiornaTabella();
}